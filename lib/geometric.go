...
//A Rect (angle) defined with 3 points A,B,C.
// AB and BC are the orthogonal edges
type Rect struct {
	A, B, C Point
}

// RectSet is a vector of points.
type RectSet struct {
	PointLists []Point
}

//Dot is the algebric dot product.
func Dot(v1, v2 Vector) int {
	return v1.X*v2.X + v1.Y*v2.Y
}

// Vectorize returns the vector AB Given 2 points A and B.
...
// InRectPlus - Given a point p1, built two oblique rects (+-45 deg) around it and check if p2 is contained
...
// IsPointIn - check if a Point is in one of the blocks contained in pointSet.
